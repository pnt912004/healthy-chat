...
def send_verification_email(to_email: str, token: str):
    print(f"Bắt đầu gửi email tới {to_email}...", flush=True)
    try:
        if not settings.SMTP_USER or not settings.SMTP_PASSWORD:
            print(f"Mock Email to {to_email}: Please verify your account at {settings.FRONTEND_URL}/verify?token={token}", flush=True)
            return
        ...
        server.quit()
        print(f"Verification email sent successfully to {to_email}", flush=True)
    except Exception as e:
        print(f"LỖI TOÀN TẬP khi gửi email tới {to_email}: {e}", flush=True)
